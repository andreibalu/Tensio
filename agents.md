...
- Severe-reading follow-up questions must also block saving that reading until the symptom answer is explicit; never let a severe reading bypass the follow-up branch while symptoms are still unanswered.
- Manual-entry persistence must fail closed: if a local save throws, keep entered values visible and show retry guidance instead of clearing the draft.
- UI must support Dynamic Type through Accessibility 3, and primary controls must be at least 56 pt tall.
- When touching Today, Log, Report, Medicines, or Settings UI, add or update at least one explicit Accessibility 3 smoke path so large-type regressions fail loudly.
